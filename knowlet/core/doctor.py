"""Pure health-check logic shared by `knowlet doctor` and `/api/doctor`.

Per ADR-0020 §Layer 5: this lives in `core/` so both CLI and web can import
it without creating a cross-layer dependency. The rendering side
(`print_doctor`) stays in `knowlet/cli/_doctor.py` since it's CLI-specific.
"""
...
def run_doctor_checks(
    ...
    *,
    ...
    (CLI) or by serializing the tuples (web).
    """
...
def _check_vault_integrity(vault: Vault) -> list[tuple[str, int, list[str]]]:
