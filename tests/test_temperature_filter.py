...
specific models. Rather than maintain a curated substring list of model ids
...
from knowlet.config import DEFAULT_LLM_MODEL, LLMConfig
...
def test_temperature_rejection_is_cached_and_retried_transparently():
    ...
    assert DEFAULT_LLM_MODEL in _no_temp_cache
...
def test_default_config_temperature_is_none():
    """The default config must not force a temperature on default-model users.
    ...
    """
    cfg = LLMConfig(api_key="stub", model=DEFAULT_LLM_MODEL)
