...
- [ ] Desktop 3 系统级入口:菜单栏 / Dock / 快捷键 / 打开最近 vault。部分完成(2026-05-30):桌面端会在 app config 中维护最近 vault 列表,启动时自动重开最近仍有效的 vault;已提供 File → Open Vault... 与 Cmd/Ctrl+O 显式切库入口。Dock 行为和显式最近 vault 列表仍待补。
