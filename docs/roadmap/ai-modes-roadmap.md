...
- [ ] B2 情绪类口吻打磨（温和、不纠错、不灌鸡汤）— 已并入 A2 的 TONE_GUIDANCE;dogfood 后据真实感受微调措辞
...
- [ ] C1 源配置（RSS/URL）+ 定时抓取（复用 mining 后端）— ~2d
- [ ] C2 digest 列表 UI（今日/本周新抓,逐条卡片）— ~1.5d
...
**剩余粗估** ≈ **12–14 单人天**（B ≈ 0.5d · C ≈ 5d · D ≈ 2d · E ≈ 4–5d · 收尾 ≈ 1.5d）。当前临时暂停主线,先做 Codex/GPT 默认迁移与文档纠偏。

**建议顺序**:A 已完成 → B2 → C → D → E。A 是引擎必须先稳;B 几乎白送;C 最重;E 垫底。
