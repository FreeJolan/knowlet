// Knowlet — Screen 1: Home (3-column, AI rail collapsed per ADR-0011)
...
const TreeFolder = ({ name, depth = 0, open = true }) => (
  <div className="kn-tree-item" style={{ paddingLeft: depth * 12 + 8, color: "var(--ink-soft)" }}>
    <I.ChevronD size={11} className="ico" style={{ color: "var(--ink-mute)" }} />
    {open ? <I.FolderOpen size={13} className="ico" /> : <I.Folder size={13} className="ico" />}
    <span style={{ flex: 1 }}>{name}</span>
  </div>
);

const TreeNote = ({ name, depth = 0, active }) => (
  <div className={`kn-tree-item${active ? " active" : ""}`} style={{ paddingLeft: depth * 12 + 8 }}>
    <span style={{ width: 11, display: "inline-block" }} />
    <I.Doc size={13} className="ico" />
...
const HomeScreen = () => {
  return (
        ...
        <section style={{ background: "var(--bg)", display: "flex", flexDirection: "column", minWidth: 0 }}>
          ...
          <div style={{
            ...
            <span className="mono" style={{ fontSize: 10.5, color: "var(--ink-mute)", padding: "0 12px" }}>
              ...
              <span>1,247 字</span>
              <span style={{ color: "var(--ink-faint)", margin: "0 6px" }}>·</span>
              <span>2026-05-01 09:42</span>
            </span>
          </div>

          {/* note body */}
          <div style={{ flex: 1, overflow: "auto", padding: "32px 64px 40px" }}>
            <div className="kn-md" style={{ maxWidth: 720, margin: "0 auto" }}>
              <h1>RAG 检索策略</h1>
    ...
    return sorted(scores.items(), key=lambda x: -x[1])`}</code></pre>
              ...
              <p>
                短查询(2–4 字)表现尚可;长查询稍弱。建议跟向量通路并用,不依赖单路。<code>jieba</code> 分词不是必须 ——
                trigram 在多数 PKM 场景已经够用。
              </p>

              <blockquote>
                Open question:cross-encoder re-rank 的延迟预算 —— 200ms p50 是 chat 体验的天花板;超出就该牺牲精度。
              </blockquote>
            </div>
          </div>
        ...
        {/* RIGHT — collapsed rail (32px per ADR-0011, 40px here for hit target) */}
        <aside style={{
          background: "var(--panel)", borderLeft: "1px solid var(--line)",
          display: "flex", flexDirection: "column", alignItems: "center",
          padding: "8px 0", gap: 4,
        }}>
          <button className="kn-icon-btn" style={{ width: 28, height: 28 }} title="大纲"><I.List /></button>
          <button className="kn-icon-btn" style={{ width: 28, height: 28 }} title="反链"><I.Link /></button>
          <button className="kn-icon-btn" style={{ width: 28, height: 28, color: "var(--accent-2)", background: "var(--accent-soft)" }} title="AI · 当前 idle">
            <I.Sparkles />
          </button>
          <span style={{ flex: 1 }} />
          <button className="kn-icon-btn" style={{ width: 28, height: 28 }} title="展开右栏"><I.ChevronR size={12} stroke={2} style={{ transform: "scaleX(-1)" }} /></button>
        </aside>
      </div>

      {/* STATUS BAR — drafts / cards / mining */}
      <footer style={{
        ...
        <button className="kn-status-btn" title="⌘⇧D 草稿审查">
          ...
          <span className="num" style={{ color: "var(--warn)", background: "rgba(196,163,115,0.10)" }}>9+</span>
        </button>
