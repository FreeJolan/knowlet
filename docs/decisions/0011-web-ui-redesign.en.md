...
- **`web/static/*` is almost entirely rewritten**: `index.html` / `app.js` / `app.css` all change; backend APIs are fully preserved (the dividend of ADR-0008).
...
- M5 / M9 Tauri desktop shell: the same `web/static` ships into Tauri unchanged, no rewrite needed.
