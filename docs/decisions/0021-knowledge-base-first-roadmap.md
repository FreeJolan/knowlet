...
> **2026-05-30 amendment**:本 ADR 的 Phase 0-4 顺序已完成其"知识库优先"纠偏使命,但不再是 AI 主线的执行清单。当前 AI 工作请看 [`docs/roadmap/ai-modes-roadmap.md`](../roadmap/ai-modes-roadmap.md);阶段 B 已 dogfood 通过,F0 AI 底层能力重构已达到当前门槛,下一步是 Stage C v2 资讯审阅与入库([设计文档](../design/stage-c-digest-inbox.md)),随后直接做 Phase 3.5 桌面端客户端;阶段 E/Quiz 暂缓到桌面端之后再评估,不是旧 Phase 3 envelope 全家桶。
